...
export class JuiceSpray {
  ...
  trigger(height: number, seed: number) {
    const random = seededRandom(seed);
    this.drops.length = 0;
    for (let i = 0; i < PRIMARY_DROPS; i++) {
      const angle = random() * Math.PI * 2;
      // A tall central jet mixed with a lower outward fan; every drop launches upward.
      const tall = i % 3 === 0;
      const lateral = tall ? .6 + random() * 1.55 : 1.5 + random() * 2.1;
      const launch = new Vector3(Math.cos(angle) * lateral, tall ? 5.9 + random() * 1.7 : 2.8 + random() * 2.6, Math.sin(angle) * lateral);
      ...
      this.drops.push({ flight, size, delay: random() * .11, spread: size * (3.1 + random() * 1.8), angle });
    }
    // Landing positions/times come from the original drops, so splashlets never spawn in mid-air.
    for (let i = 0; i < SECONDARY_DROPS; i++) {
      const parent = this.drops[Math.floor(i / 2) * 3 + 1];
      const angle = parent.angle + (random() - .5) * 2.8, speed = .8 + random() * 1.1;
      const origin = parent.flight.impact.clone(); origin.y += .008;
      const flight = new JuiceFlight(origin, new Vector3(Math.cos(angle) * speed, 1 + random() * 1.65, Math.sin(angle) * speed), .85);
  ...
  update(time: number, reduced: boolean) {
    ...
    const primaryCount = reduced ? 48 : PRIMARY_DROPS;
  ...
  reset() { this.root.visible = false; this.drops.length = 0; this.spray.count = this.stains.count = 0; }
}
