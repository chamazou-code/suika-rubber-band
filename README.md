...
メーターを狙って、輪ゴムを1本。スイカの限界まで巻く、ワンタップのブラウザゲーム。v1.2.4。
...
- 上部がまず上へ飛び、大小6つの皮付き破片に分かれ、果肉がさらに飛び散ります。本数が多いほど勢いと飛沫量が増え、噴き上がり・広がり・回転・飛散方向は毎回変わります。下部は欠けた皮と砕けた果肉を見せてテーブルに残り、輪ゴムも解放されて飛散します。人物は登場しません。すべてスイカの果汁・果肉で、血液や怪我の表現はありません。
...
PerspectiveCameraで約36度の俯瞰。皮の縞・艶・凹凸と温かい照明で立体感を作り、中央だけを局所的にくびれさせます。破裂後の断面は凹凸のある三角形メッシュで、皮の欠けと果肉の窪みを表現します。解放された下半分は0.7秒でくびれが戻り、切り口が広がって少し沈みます。底の接地点は動かしません。1/120秒刻みの軽い物理計算で、破片と輪ゴムに重力・回転・床への反発・摩擦を適用します。輪ゴムは解放されると縮み、果肉より弾みます。輪ゴムと粒子はInstancedMesh。輪ゴムは最大70本、本数に応じて果肉28〜48個・皮8〜14個・種18〜28個、果汁84〜144滴と着地時の跳ね返り56〜96滴を描画します。果汁跡は144個を上限に固定確保します。果汁だけは重力と空気抵抗から軌道・着地時刻を直接求め、剛体計算を増やしません。空中で砕ける上半分から果肉を追加放出し、着地した汁は不規則な濡れ跡として広がります。

破裂時に最終本数から勢い・粒子数を決め、描画専用の乱数で噴き上がり型・広がり型・回転型と軌道を選びます。毎回新しく抽選し、ページを開き直しても固定の順番にはなりません。耐久値・隠しダメージ・メーターの抽選には影響しません。上半分と果肉・汁は同じ設定を共有し、分裂の瞬間に位置が飛ばないようにしています。飛距離はテーブルの範囲に収め、reduced-motionでは飛沫を通常の約1/3、最大48滴に抑えます。
...
`node scripts/verify-intensity.mjs` で15／35／55／70本と同本数の3パターンをスマホ・PC幅で比較し、分裂前後の連続性を検証できます。

`node scripts/verify-juice.mjs after` でスマホ幅・PC幅の爆発画像、汁の着地・reduced-motion・再挑戦時の初期化、CPU更新時間を検証できます。
